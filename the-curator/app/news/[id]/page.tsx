    ...
    <div className="bg-slate-50 min-h-screen">
      <div className="mx-auto max-w-4xl space-y-6 px-4 py-10 sm:px-6 lg:px-8">
        ...
        <article className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {article.author && (
            <p className="text-sm text-slate-700">撰文：{article.author}</p>
          )}
          {article.main_image_url && (
            <figure className="rounded-2xl border border-slate-100 bg-slate-100/80 overflow-hidden">
              <img
              ...
              {article.main_image_caption && (
                <figcaption className="p-3 text-xs text-slate-600">{article.main_image_caption}</figcaption>
              )}
            </figure>
          )}
          <div className="space-y-4 text-base text-slate-700">
            {article.content?.map((block, index) =>
              block.type === 'heading' ? (
                <h3 key={`${block.text}-${index}`} className="text-xl font-semibold text-slate-900">
                  {block.text}
                </h3>
              ) : (
              ...
              {article.tags
                ...
                .map((tag) => (
                  <Link
                    ...
                    className="rounded-full border border-slate-200 px-3 py-1 transition-colors hover:border-slate-900 hover:text-slate-900"
                  >
                    {tag}
                  </Link>
                ))}
